...
const CamCat = ({ catId }) => {
	...
	const getPosts = async () => {
		const res = await axios.get(`/api/cam/${catId}`);
		if (res.data !== null) {
			setPosts(res.data);
		}
	};
	const getCat = async () => {
		const res = await axios.get(`/api/cam/categories/${catId}`);
	...
	const submitNewHandler = async (e) => {
		...
		await axios.post(`/api/cam`, data, options);
		setMessage("");
		setTitle("");
		setText("");
		setZip("");
	...
	const editHandler = async (id) => {
		...
		const res = await axios.get(`/api/cam/post/${id}`);
	...
	const updateForm = async (e) => {
		...
		await axios.put(`/api/cam/${id}`, data, options);
		setMessage("");
		getPosts();
		setIsLoading(false);
		setProgressModal(false);
